...
- Name: dashboard.spec.ts >> EMBER Dashboard >> Risk Gauge >> displays numeric risk score
- Location: tests\e2e\dashboard.spec.ts:28:5
...
Locator: getByTestId('risk-gauge').locator('text').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByTestId('risk-gauge').locator('text').first()
